...
; принимает значение и оставшийся хвост и возвращает результат
(def -valid? boolean)
; правда ли, что получился корректный результат
(def -value :value)
; возвращает :value
(def -tail :tail)
; возвращает :tail
...
; если результат валидный, то функция применяется к результату
    ...
    (let [ar ((force a) str)]
      (if (-valid? ar)
        ((_map (partial f (-value ar)))
          ((force b) (-tail ar)))))))
; парсим первым парсером, затем вторым

(defn _either [a b]
  (fn [str]
    (let [ar ((force a) str)]
      (if (-valid? ar) ar ((force b) str)))))
; пытаемся попарсить первым парсером, если не выходит, то парсим вторым
...
; проверяет на совпадение символов, но при этом символы передаются списком
(defn +char-not [chars] (_char (comp not (set chars))))
(defn +map [f parser] (comp (_map f) parser))
; применяет _map к парсеру
(def +ignore (partial +map (constantly 'ignore)))
; игнорирует результат данного парсера
...
; применяет последовательность парсеров к результату
